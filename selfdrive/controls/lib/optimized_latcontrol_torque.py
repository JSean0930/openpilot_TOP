...
# 計算速度自適應因子
speed_factor = max(0.5, min(1.5, 1.0 + 0.1 * (v_ego / 20)))
...
class LatControlTorque(LatControl):
  def __init__(self, CP, CI):
    ...
    self.pid = PIDController(self.torque_params.kp * speed_factor, self.torque_params.ki * speed_factor, k_f=self.torque_params.kf * speed_factor, pos_limit=self.steer_max, neg_limit=-self.steer_max)
    ...
    if self.use_nn:
      self.pitch = FirstOrderFilter(0.0, max(0.3, min(0.7, 0.5 + 0.2 * (v_ego / 15))), 0.01)
