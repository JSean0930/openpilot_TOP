...
#優化 cost function / warm start / 前車預測優化 / 降低計算負擔 / E2E 模式下起步跟隨靈敏度
...
# === E2E 起步跟隨靈敏度設定 ===
E2E_START_FOLLOW_SENSITIVITY = 1.2  # 當速度變化超過此值，迅速跟隨
...
def gen_long_ocp():
  ...
  # === Cost Function 改善 ===
  # 動態調整權重，根據距離與速度平衡成本
  decay_factor = np.linspace(1, 0.2, N)
  ocp.cost.W = np.diag([1.0, 0.8, 0.2, 0.5, 0.1, 0.05]) * decay_factor[:, None]
  ...
  # === Warm Start 優化 ===
  for i in range(N+1):
    if hasattr(self, 'prev_x_sol'):
      self.solver.set(i, 'x', self.prev_x_sol[i])
    if i < N and hasattr(self, 'prev_u_sol'):
      self.solver.set(i, 'u', self.prev_u_sol[i])
...
class LongitudinalMpc:
  ...
  for i, t in enumerate(T_IDXS):
    v_pred = max(0.0, v_lead + a_lead * t)
    x_pred = x_lead + v_lead * t + 0.5 * a_lead * t**2
    x_lead_0[i] = x_pred + lead_buffer
    v_lead_0[i] = v_pred

  return np.column_stack((x_lead_0, v_lead_0))
...
# 更新步數根據速度
self.N = get_dynamic_n(v_ego)

# 更新動態變數（緩存處理）
if not hasattr(self, 'T_IDXS') or len(self.T_IDXS) != self.N + 1:
    self.T_IDXS = np.linspace(0.0, MAX_T, self.N + 1)
    self.T_DIFFS = np.diff(self.T_IDXS, prepend=[0.])
...
# === E2E 模式下的起步跟隨靈敏度 ===
if self.source == 'e2e' and (v_lead0 - v_ego > E2E_START_FOLLOW_SENSITIVITY or v_lead1 - v_ego > E2E_START_FOLLOW_SENSITIVITY):
    # 起步跟隨，允許更快反應
    self.params[:,5] = LEAD_DANGER_FACTOR * 1.5
    self.source = 'lead0' if v_lead0 > v_lead1 else 'lead1'
else:
    # 維持當前跟隨策略
