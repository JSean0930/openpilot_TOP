...
class LongitudinalMpc:
  ...
  def set_weights(self, prev_accel_constraint=True, personality=log.LongitudinalPersonality.standard, v_lead0=0, v_lead1=0):
    ...
    if self.mode == 'acc':
      ...
      cost_weights = [1., 0.1, 0.2, 5.0, a_change_cost * a_change_v_ego, 1.0]
      constraint_cost_weights = [LIMIT_COST, LIMIT_COST, LIMIT_COST, DANGER_ZONE_COST]
    else:
  ...
  def update(self, radarstate, v_cruise, x, v, a, j, personality=log.LongitudinalPersonality.standard, dynamic_follow=False):
    ...
    if self.mode == 'acc':
      ...
      lead_safe_1 = lead_xv_1[:,0] + get_safe_obstacle_distance(lead_xv_1[:,1], t_follow, stop_distance)
      lead_safe = np.minimum(lead_safe_0, lead_safe_1)
      # clamp 掉任何小于安全距离的部分
