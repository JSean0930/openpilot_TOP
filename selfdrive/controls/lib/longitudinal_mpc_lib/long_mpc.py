  ...
  def update(self, radarstate, v_cruise, x, v, a, j, personality=log.LongitudinalPersonality.standard, dynamic_follow=False):
    ...
    if self.mode == 'acc':
      ...
      x_obstacles = np.column_stack([lead_0_obstacle,
                                     lead_1_obstacle])
      # cruise 目標距離
      cruise_target = T_IDXS * np.clip(v_cruise * 0, v_ego - 2.0, 1e3) + x[0] # *1.0是放大係數（可改為 >1.0 讓巡航更激進，或 <1.0 更保守），下限 v_ego - 2.0 決定了當車速高於目標時是否允許輕微減速。
      ...
      x_and_cruise = np.column_stack([x * 0.9, cruise_target]) # 將 e2e 預測距離額外乘以 0.95，會讓 e2e 軌跡對加速目標略顯保守。數值越接近 1，e2e 的影響越大；越小，則更偏向 cruise，進而影響加速決策和引擎轉速。
      #x = np.max(x_and_cruise, axis=1)
      #計算速度加權：低速偏 e2e，高速偏 cruise
      w = np.clip((v_ego - 5.0) / 28.0, 0.0, 0.3)  #15
      x = (1 - w) * np.min(x_and_cruise, axis=1) + w * np.max(x_and_cruise, axis=1) * 1.0
