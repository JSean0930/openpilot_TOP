...
class LongitudinalMpc:
  ...
  def reset(self):
    ...
    self.yref = np.zeros((N+1, COST_DIM))  # 保留資料結構但不設定進 solver
...
def update(self, radarstate, v_cruise, x, v, a, j, v_ego, v_lead, a_lead, personality=log.LongitudinalPersonality.standard, dynamic_follow=False):
  ...
  def run(self):
    ...
    self.prev_a = np.interp(T_IDXS + self.dt, T_IDXS, self.a_solution, left=self.a_solution[0], right=self.a_solution[-1])
