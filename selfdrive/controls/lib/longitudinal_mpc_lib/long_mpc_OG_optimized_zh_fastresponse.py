...
# === 原始成本設定備份 ===
ORIGINAL_J_EGO_COST = 5.0
ORIGINAL_A_CHANGE_COST = 200.0
ORIGINAL_T_IDXS = np.array([...])  # 原始請參照上方原始碼



# === 自適應速度差權重調整（使用較近的 lead） ===
lead0_dist = lead0.dRel if lead0 is not None else 1e6
lead1_dist = lead1.dRel if lead1 is not None else 1e6
closest_lead = lead0 if lead0_dist <= lead1_dist else lead1

if closest_lead is not None:
    rel_speed = closest_lead.vLead - v_ego
else:
    rel_speed = 0.0  # default

if rel_speed < -2.0:
    A_EGO_COST = 10.0  # 積極減速
else:
    A_EGO_COST = 1.0   # 正常情況
...
J_EGO_COST = 1.0  # 降低 jerk 懲罰以提高反應靈敏度
A_CHANGE_COST = 50.0  # 降低以提供更大加速自由度
...
T_IDXS = np.array([0.0, 0.2, 0.5, 0.9, 1.5, 2.5, 4.0, 5.5, 7.0])
FCW_IDXS = T_IDXS < 5.0
