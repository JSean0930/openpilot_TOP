...
def get_dynamic_follow(v_ego, v_lead, a_lead=0.0, personality=log.LongitudinalPersonality.standard):
  ...
  if personality == log.LongitudinalPersonality.relaxed:
    min_dist, max_dist = 1.2, 1.5
  elif personality == log.LongitudinalPersonality.standard:
    min_dist, max_dist = 1.1, 1.4
  elif personality == log.LongitudinalPersonality.aggressive:
    min_dist, max_dist = 1.0, 1.3
  else:
...
class LongitudinalMpc:
  ...
  def update(self, radarstate, v_cruise, x, v, a, j, v_lead, a_lead, personality=log.LongitudinalPersonality.standard, dynamic_follow=False):
    ...
    if self.mode == 'acc':
      ...
      w = np.clip((v_ego - 5.0) / 10.0, 0.0, 1.0)  #15, 提高 5.0 → 讓系統更晚開始過渡到 cruise, 降低 10.0 → 過渡更陡峭，變化更快
