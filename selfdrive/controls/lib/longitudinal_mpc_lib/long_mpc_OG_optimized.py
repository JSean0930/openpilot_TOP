...
class LongitudinalMpc:
  ...
  def update(self, radarstate, v_cruise, x, v, a, j, personality=log.LongitudinalPersonality.standard, dynamic_follow=False):
    ...
    # === E2E 起步靈敏度判斷 ===
    if self.source == 'e2e' and (lead_xv_0[0,1] - v_ego > E2E_START_FOLLOW_SENSITIVITY or lead_xv_1[0,1] - v_ego > E2E_START_FOLLOW_SENSITIVITY):
        self.source = 'lead0' if lead_xv_0[0,1] > lead_xv_1[0,1] else 'lead1'
  ...
  def run(self):
    ...
    # 儲存本輪解答供下一輪 Warm Start 使用
    for i in range(N+1):
        self.prev_x_sol[i] = self.solver.get(i, 'x')
    for i in range(N):
        self.prev_u_sol[i] = self.solver.get(i, 'u')
