...
# === 根據速差調整成本權重 ===
    lead = lead0 if lead0['dRel'] < lead1['dRel'] else lead1
    v_lead = lead['vLead']
    v_rel = v_lead - v_ego  # 速差（正值：前車加速）

    # 設定各成本的基本值與範圍
    base_a_ego_cost = 0.0
    base_a_change_cost = 100.0
    base_x_ego_cost = 0.0
    base_j_ego_cost = 5.0

    min_a_change_cost = 50.0
    max_a_change_cost = 300.0

    speed_diff_factor = np.clip(1 - v_rel / 5.0, 0.0, 1.0)

    A_EGO_COST = base_a_ego_cost
    A_CHANGE_COST = base_a_change_cost + speed_diff_factor * (max_a_change_cost - base_a_change_cost)
    X_EGO_COST = base_x_ego_cost
    J_EGO_COST = base_j_ego_cost + (1.0 - speed_diff_factor) * 10.0  # 速差大時讓jerk更小
